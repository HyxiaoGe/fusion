...
cat "${GITHUB_WORKSPACE}/ops/deploy/prompt-hold-preflight.py" \
  "${GITHUB_WORKSPACE}/ops/deploy/prompt-bundle-smoke.py" \
  | docker exec -i -e FUSION_ROLLBACK_REQUESTED="${ROLLBACK_REQUESTED:-false}" fusion-api python -
