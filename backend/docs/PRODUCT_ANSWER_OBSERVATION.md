...
**新增交付延迟：`_STORE_WAIT_SECONDS = 1.0` 的等待发生在产品回答交付路径上。** 相比原先只有日志写入，数据库卡顿或写入排队时，每轮产品回答最多会额外等待 1 秒再交付。这是为等待观测落库新增的用户可见延迟；超时后继续交付答案，已接收观测仍可排队完成。
...
迁移只创建空表，不导入或重建 #71 的旧 17 条日志。**迁移完成不等于真实观测期已开始。** 部署后需先核对新代码、迁移和实际落库，并以实际新记录确定新观测期起点；`retained_range.first` 是所查范围内首条留存记录，不是流量开始时间或完整留存证明。

**新旧两段观测不能合并查询：2026-09-04 至 2026-09-19 的旧 17 条只在 `app.log` 中，不会回填新表。** 新表只记录部署后的新观测；聚合 API 不读取旧日志，不能用一次新表查询覆盖两段观测。`retained_range` 只反映查询时间范围内的表内记录，不会提示表外还存在旧日志期；查询“观测期以来”时必须另行说明这段历史边界，不能把新表首条记录当成整条历史的起点。
