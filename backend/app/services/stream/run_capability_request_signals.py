...
# 「是不是真的」也可描述付费意愿等普通语境，不能独立作为主张标记；
# 「这个说法是不是真的」仍由「说法」识别。见 issue #98。
_EXTERNAL_CLAIM_OBJECT = (
    r"(?:说法|消息|新闻|报道|传闻|爆料|公告|声明|通告|来源|出处|原文|"
    r"真伪|真假|真实性|准确性|可信度|是否属实|属实|"
...
# 单字「别」只作命令否定；分别、类别、告别等词尾不能撤销工具授权。
_STANDALONE_BIE = r"(?<![分个特区差性级辨识鉴判甄类国告离送惜永诀阔道作])别"
