...
class AmapProductDefinitionTests(unittest.TestCase):
    def test_definitions_are_stable_closed_product_contracts(self):
        ...
        self.assertIn("Pass only when explicitly specified", departure_description)
        self.assertIn("otherwise omit it", departure_description)
        self.assertIn("do not default to the present", departure_description)
        ...
        self.assertIn("Omit it when unspecified", route_description)
...
class AmapWeatherForecastTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_named_location_requires_city_confirmation_without_guessing(self):
        ...
        self.assertIn("ask the user directly", context)
...
class AmapRouteCompareTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_local_place_context_contains_strict_evidence_rules_and_remains_escaped_and_bounded(self):
        ...
        self.assertIn("Refer only to places actually returned in result.places", context)
        self.assertIn("do not introduce a place absent from result.places", context)
        self.assertIn("this query's results cannot confirm it", context)
        self.assertIn("Do not infer real-time queues, availability, reservations", context)
        self.assertIn("walking time between places, or distance between places", context)
        self.assertIn("reference_cost_yuan is reference spending, not per-person spending", context)
        self.assertIn("only when that place actually returned distance_m", context)
        self.assertIn("distance from this query's anchor/near", context)
        self.assertIn("Give the conclusion first", context)
        self.assertIn("conditional recommendation", context)
        self.assertIn("3 to 5 short paragraphs", context)
        self.assertIn("no table", context)
        self.assertIn("mechanically joining card fields", context)
        self.assertIn("untrusted external data", context)
    ...
    def test_route_context_contains_strict_evidence_rules_and_remains_bounded(self):
        ...
        self.assertIn("Refer only to routes actually returned in result.routes", context)
        self.assertIn("do not introduce an absent route or transport mode", context)
        self.assertIn("this query's results cannot confirm it", context)
        self.assertIn("duration_s actually returned by result.routes and distance_m for non-transit", context)
        self.assertIn("route.distance is walking distance at the origin and destination", context)
        self.assertIn("Do not estimate route time or distance", context)
        self.assertIn("Give the conclusion first", context)
        self.assertIn("conditional recommendation", context)
        self.assertIn("3 to 5 short paragraphs", context)
        self.assertIn("no table", context)
