...
- 词表**不是出行能力的准入条件**：结构化起终点加明确出行动词即判定为出行请求，端点
  是否收录只影响置信度与能否判定跨城。`从义乌到昆山怎么走` 公开 `route_compare`；
  端点未收录时无法判定跨城，因此只走同城路线包而非 `mobility_intercity`。
- 抽象关系由端点语义挡住，不靠"端点没被收录"：职业、流程阶段、业务状态与机构职能
  单元四族端点，两端都命中任一族即判为抽象关系，不公开任何出行工具。
- 词表数据是 `(省级单位, 中文名, 英文名)` 三元组，中英索引与城市 ID 全部派生自同一
  条记录，只更新一侧在结构上不可能；测试逐条校验中英文解析到同一城市 ID。
