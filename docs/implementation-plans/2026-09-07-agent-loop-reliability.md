...
- [ ] 写失败测试：真实 SQLite 新 Session 校验 checkpoint 与取消/失败后 search 仍存在，客户端伪造 search 仍被拒绝；同代正常完成作对照。
- [ ] 写失败测试：一个真实 executor 子任务抛 StreamOwnershipLostError，另一个受控工具必须先完成取消清理，批次再抛原异常。增加正常并发和普通取消对照。
- [ ] 写失败测试：合法计划中地点完成、天气待办，分别触发 max_steps/max_tool_calls/timeout；收集真实输出正文，断言包含已知地点及未完成提示，没有新增模型/工具调用或 plan_execution_repair。
- [ ] 运行新增测试，确认失败来自目标行为；复现参考 `/private/tmp/fusion_loop_partial_sqlite_repro.py`、`/private/tmp/fusion_loop_gather_repro.py`、`/private/tmp/fusion-loop-review-y8m85vjg/review/test_terminal_output.py`，正式测试必须自包含。
- [ ] 实现最小修复：可信服务器持久化保留新增结果；并发异常清理后重抛；触顶走不再请求后续执行的终局路径。
- [ ] 运行 persistence、tool executor/tool round、driver/outcome、run completion 目标测试与改动文件 Ruff，检查 diff。
- [ ] 精确暂存本任务文件并中文本地提交；写报告与提交 SHA，接受独立审查。
...
- [ ] 写失败测试：生产 round/outcome 输出路径中普通模型正文、延迟采用、产品与知识库替换、计划抑制分别产生正确归因；取消/失败后已有可见内容不被错误标为未输出。
- [ ] 写失败测试：真实事件白名单和历史详情查询保留归因、隔离跨会话/跨轮数据；旧历史缺失时为未知。事件不包含候选/答案全文。
- [ ] 写前端失败测试：现有详情面板显示“已输出 / 未输出 / 已改写或替换”的状态、来源及原因；模型候选仍可查看；旧响应不伪造采用状态。
- [ ] 在实际输出与策略决策处发出事件，扩展现有安全投影和详情 DTO；非模型收尾不关联前一轮，不新增数据库表/列。新增观测 sink 错误保留既有运行异常和降级方式。
- [ ] 贯通前端 live/history 接收和详情展示，使用现有 API、Redux/查询链路及中英文资源。
- [ ] 运行目标 pytest/Vitest/Ruff/ESLint，执行前端类型检查和生产构建；若存在基线问题，使用同 SHA 干净树核对，不能静默忽略。
- [ ] 精确暂存本任务文件并中文本地提交；写报告与提交 SHA，接受独立审查。

### Task 3: 综合验证与交付

- [ ] 主代理亲自读取最终差异并重跑跨任务目标集合，检查已有成功路径与新增边界。
- [ ] 独立审查整个分支的规格符合性与可达正确性问题，修复后复核。
- [ ] 更新规格验收记录与 `docs/EXECUTION_LEDGER.md`，准确标明本地代码证据、未推送/部署及真实环境未验收。
- [ ] 检查 `git diff --check`、工作区与提交范围，保留分支/worktree 供用户审阅。
