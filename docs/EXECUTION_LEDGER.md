...
## 2026-09-26 语义正则迁移第一阶段（本地验证完成，发布待核）

- 请求文本上的计划工具、搜索意图、火车类型和网络否定词匹配改由现有模型输出结构化约束，服务端继续校验并裁剪执行边界；动态发现仅在首次 `tool_search` 声明后冻结授权目录。
- 本地架构、Ruff、格式与差异检查通过；按 CI 收集入口 3242 项 unittest 通过、2 项跳过。dev 同代理别名 5 条新提示词探针可解析，但不是新服务真实 API 验收。发布身份、工具轨迹和第二阶段产品答案迁移仍待核。详见[阶段一报告](reports/backend/2026-09-26-semantic-regex-migration-stage1.md)。

## 2026-09-26 语义正则迁移第二阶段（本地验证完成，发布待核）

- 第一阶段 PR #148 的 required gate 全绿，合并提交 `676c9fed`；对应 dev 发布尚在执行，不把合并当部署成功。
- 产品答案的主观措辞判断改由现有最终回答模型处理，保留可直接核对的硬事实边界及事实兜底。与第一阶段组合后 3245 项 unittest 通过、2 项跳过，CI 额外 241 项 pytest 通过。
- dev 同别名天气、单类车次、空铁各 2 次提示词层探针未见越界；真实 Fusion API、运行镜像和页面仍待发布后验收。详见[阶段二报告](reports/backend/2026-09-26-semantic-regex-migration-stage2.md)。
