# Fusion Monorepo 协作导航

本文件只负责把协作者路由到权威约定，不重复应用实施规则。

- 修改 `backend/`：先读 [backend/AGENTS.md](backend/AGENTS.md)；使用 Claude 时同时读 [backend/CLAUDE.md](backend/CLAUDE.md)。
- 修改 `frontend/`：先读 [frontend/AGENTS.md](frontend/AGENTS.md)；使用 Claude 时同时读 [frontend/CLAUDE.md](frontend/CLAUDE.md)。
