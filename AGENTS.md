...
- 缺失信息、不可逆后果或权限边界需要用户决定时，暂停依赖它的操作并先完成独立工作。有证据表明用户一次简短操作明显比代理继续尝试更省时、少耗配额时，交接最小必要步骤，完成后接回任务；不要把普通可自行修复的错误交给用户。
- 因 skill 要求额外确认、停工或偏离任务时，指出实际读取的文件、相关原文与适用原因，区分明文要求和自行推断；先核对用户已有授权，通用 skill 不覆盖用户明确要求。工具审批阻止执行时说明真实阻塞，不改走其他通道绕过。
...
跨应用或协议改动读取两侧约定，小改动只读取相关入口。按具体任务选择 skill，不能仅因出现 Git、网页或数据库关键词就加载完整开发流程。架构资料、计划和历史报告按问题需要查找，不要求每次遍历仓库。

## 验证与完成

- 先明确可观察的预期行为。bug 修复应有能暴露原故障的回归证据；纯文档、低风险样式或可逆配置按实际风险验证，不编写复刻实现的测试。
- 以当前需求的最小完整方案为默认，沿现有架构修改。新增抽象、依赖或重构必须解决已确认的问题并能说明收益；不为假设中的扩展先建框架，也不以少改代码为由掩盖根因。
...
修改 skill 的触发条件、授权或工具选择规则时，按需复核[协作行为场景](docs/development-guidance-scenarios.md)中的相关案例。文档结构检查、模拟行为与真实工具验证分别记录，不把静态检查通过当作代理行为保证。
