...
describe('TrajectoryCellProjection', () => {
  ...
  it('将同一 LLM round 合并为会话级 Request 编号，并关联快照正文预览', () => {
          ...
          events: [
            ...
            event('selected-llm', 2, 'llm_round_completed', {
              payload: {
                ...
                output_tokens: 40,
                reasoning_tokens: 24,
                output_provenance: { disposition: 'replaced', source: 'server', reason: 'product_guard', block_id: 'text-1' },
                duration_ms: 800,
                ttft_ms: 90,
              },
            }),
    ...
    expect(projection.cells.find(
      ...
      outputTokens: 40,
      reasoningTokens: 24,
      outputProvenance: { disposition: 'replaced', source: 'server', reason: 'product_guard', block_id: 'text-1' },
      durationMs: 800,
      ttftMs: 90,
